	...
	static function register_menus()
	{
		...
		foreach ($handlers as $key => $handler) {
			...
			if (isset($handler['post_type']) && isset($handler['@service']) && $handler['@service'] === true) {
				if (!isset($handler['service_label']))
					continue;
		...
		foreach ($registered_apps as $key => $app) {
			add_menu_page($app['name'], $app['name'] . ' App', 'manage_options', 'awesome-app-' . $app['slug'], function () use ($app) {
				apps_setup_wp::show_app_pages($app); }, 'dashicons-admin-multisite', 3);
